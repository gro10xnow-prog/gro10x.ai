 ...
 *   node scripts/e2e-client/runner.js --phase 4   (Runs specific phase)
 */
const { startServerIfNeeded, launchBrowser, wait, REPORTS_DIR } = require('./utils');
const { generateReport } = require('./report');

// Import Phase Modules
const { runPhase1 } = require('./phases/phase-1-auth');
const { runPhase2 } = require('./phases/phase-2-miniapp');
const { runPhase3 } = require('./phases/phase-3-partners');
const { runPhase4 } = require('./phases/phase-4-spa');
const { runPhase5 } = require('./phases/phase-5-bot');
const { runPhase6 } = require('./phases/phase-6-sse');
const { runPhase7 } = require('./phases/phase-7-edge-cases');
const { runPhase8 } = require('./phases/phase-8-responsive');
const { runPhase9 } = require('./phases/phase-9-cross-browser');
...
async function main() {
  ...
  try {
    ...
    if (!targetPhase || targetPhase === 5) {
      phaseResults.push(await runPhase5());
    }
    if (!targetPhase || targetPhase === 6) {
      phaseResults.push(await runPhase6());
    }
    if (!targetPhase || targetPhase === 7) {
      phaseResults.push(await runPhase7());
    }
    ...
    console.log('\n============================================================');
    console.log('📊 Generating Comprehensive Client Test Report...');
    ...
    if (jsonSummary.totalFailed > 0) {
      console.warn(`⚠️ Warning: ${jsonSummary.totalFailed} tests failed during execution.`);
    }

  } catch (err) {
    console.error('❌ Client E2E Runner Fatal Error:', err);
  } finally {
    await browser.close();
    if (serverInstance) {
