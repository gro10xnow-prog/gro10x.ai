...
window.APP_MODULES.dashboard = async function(container) {
      ...
      container.innerHTML = `
        ...
        <div style="background: linear-gradient(135deg, rgba(0, 223, 137, 0.12), rgba(6, 182, 212, 0.08)); border: 1px solid rgba(0, 223, 137, 0.25); border-radius: 20px; padding: 1.5rem; margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
          ...
          <div style="display: flex; gap: 0.6rem; align-items: center; flex-wrap: wrap;">
            ...
            <a href="#kanban" onclick="setTimeout(() => window.KANBAN_MODULE?.openNewTaskModal?.(), 300);" class="btn-secondary" style="font-size: 0.8rem; text-decoration: none;" title="Open Production Pipeline Hub">📋 New Task</a>
            <a href="#finance" onclick="setTimeout(() => window.FINANCE_MODULE?.openNewInvoiceModal?.(), 300);" class="btn-secondary" style="font-size: 0.8rem; text-decoration: none;" title="Create New Client Invoice">🧾 New Invoice</a>
        ...
        <div style="background: var(--surface-1, #0f172a); border: 1px solid rgba(0, 223, 137, 0.25); border-radius: 18px; padding: 1.25rem; margin-bottom: 1.5rem;">
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
            ...
            <a href="#gigs" style="text-decoration:none; color:inherit; display:block; padding:0.85rem; border-radius:14px; background:var(--surface-2, #162032); border:1px solid var(--border-subtle); transition:all 0.2s ease;" onmouseover="this.style.borderColor='var(--cyan-brand)'; this.style.transform='translateY(-2px)'" onmouseout="this.style.borderColor='var(--border-subtle)'; this.style.transform='none'" title="Manage Marketplace Gigs & Freelance Platforms">
              <div style="display:flex; justify-content:space-between; font-size:0.82rem; font-weight:700; margin-bottom:0.35rem;">
                <span style="color:var(--text-primary); display:flex; align-items:center; gap:0.25rem;">⚡ 2. Platform Revenue (25%) <span style="font-size:0.7rem; color:var(--text-dim);">↗</span></span>
              ...
              <div style="font-size:0.72rem; color:var(--text-muted); margin-top:0.35rem;">Fiverr (4 Gigs Live) · Chrome Store</div>
            </a>

            <a href="#digistore" style="text-decoration:none; color:inherit; display:block; padding:0.85rem; border-radius:14px; background:var(--surface-2, #162032); border:1px solid var(--border-subtle); transition:all 0.2s ease;" onmouseover="this.style.borderColor='#f59e0b'; this.style.transform='translateY(-2px)'" onmouseout="this.style.borderColor='var(--border-subtle)'; this.style.transform='none'" title="Manage DigiVault & Digital Products">
              <div style="display:flex; justify-content:space-between; font-size:0.82rem; font-weight:700; margin-bottom:0.35rem;">
              ...
              <div style="font-size:0.72rem; color:var(--text-muted); margin-top:0.35rem;">DigiVault (AI Subs) · 13 Etsy Stores</div>
            </a>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 1rem; margin-bottom: 1.5rem;">
          ...
          <a href="#leads" class="kpi-tile" style="border:1px solid var(--border-subtle); text-decoration:none; color:inherit; transition:all 0.2s ease;" onmouseover="this.style.borderColor='var(--cyan-brand)'" onmouseout="this.style.borderColor='var(--border-subtle)'" title="Open CRM Pipeline & Inquiries">
            ...
            <div class="kpi-val" style="color: var(--cyan-brand);">${isUSD ? `$${Math.round(totalLeadVal).toLocaleString()}` : formatBDT(totalLeadVal * 118)}</div>
          ...
          <div class="card-glass" style="background: var(--surface-1, #0f172a); border: 1px solid var(--border-subtle); border-radius: 18px; padding: 1.25rem;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 1rem; border-bottom: 1px solid var(--border-subtle); padding-bottom: 0.5rem;">
              <div>
                ...
                <div style="font-size: 0.72rem; color: var(--text-muted); margin-top: 0.2rem;">${leads.length} Inquiries · Pipeline: ${isUSD ? `$${Math.round(totalLeadVal).toLocaleString()}` : formatBDT(totalLeadVal * 118)}</div>
              </div>
                      ...
                      <tr>
                        ...
                        <td>
                          <div style="display: flex; align-items: center; gap: 0.45rem;">
                            ${cleanPhone.length >= 7 ? `<a href="https://wa.me/${cleanPhone}" target="_blank" rel="noopener noreferrer" style="color:var(--brand-primary); text-decoration:none; font-weight:700;">💬 WA</a>` : ''}
                            ${l.email ? `<a href="mailto:${escapeHTML(l.email)}" style="color:var(--cyan-brand); text-decoration:none; font-weight:700;">✉️ Email</a>` : ''}
                            ${cleanPhone.length < 7 && !l.email ? `<span style="color:var(--text-muted);">-</span>` : ''}
                          </div>
                        </td>
                      </tr>
                    `;}).join('')}
          ...
          <div class="card-glass" style="background: var(--surface-1, #0f172a); border: 1px solid var(--border-subtle); border-radius: 18px; padding: 1.25rem;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 1rem; border-bottom: 1px solid var(--border-subtle); padding-bottom: 0.5rem;">
              <div>
                ...
                <div style="font-size: 0.72rem; color: var(--text-muted); margin-top: 0.2rem;">${collectionRate}% Collection Rate · ${isUSD ? '$' + paidTotal.toLocaleString() : formatBDT(paidTotal * 118)} Settled</div>
              </div>
            ...
            ${invoices.length === 0 ? `
              <div style="padding: 1.5rem; text-align: center; color: var(--text-muted);">
                ...
                <button onclick="if(window.openNewInvoiceModal){window.openNewInvoiceModal();}else{location.hash='#finance';}" style="background:var(--brand-primary); color:#000; border:none; padding:0.4rem 0.8rem; border-radius:8px; font-size:0.75rem; font-weight:700; cursor:pointer;">+ Create First Invoice</button>
              </div>
            ` : `
              <div class="table-responsive">
                <table class="data-table" style="font-size: 0.8rem; width:100%;">
  ...
  window.execRejectExpense = async function(id) {
    if (!confirm('Are you sure you want to reject this expense claim?')) return;
    try {
      if (window.APP_API) {
  ...
  window.execRejectLeave = async function(id) {
    if (!confirm('Are you sure you want to decline this leave application?')) return;
    try {
      if (window.APP_API) {
  ...
  // Cross-tab and module currency sync
