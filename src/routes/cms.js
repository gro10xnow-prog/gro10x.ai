...
// GET Public Landing Page CMS Content
router.get(['/', '/content', '/public/content'], (req, res) => {
  const db = readDB();
...
router.put('/', requireAuth, requireAdmin, (req, res) => {
  const db = readDB();
  db.cmsContent = { ...defaultCMSContent, ...(db.cmsContent || {}), ...req.body, updatedAt: new Date().toISOString() };
  writeDB(db);
  broadcast('cms_update', db.cmsContent);
